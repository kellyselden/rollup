	...
	it('fails with invalid keys', () => {
		return rollup.rollup({ input: 'x', plUgins: [] }).then(
			...
			err => {
				assert.equal(
					err.message,
					"Unexpected key 'plUgins' found, expected one of: acorn, amd, banner, cache, context, entry, exports, extend, external, file, footer, format, freeze, globals, includeMissingExports, indent, input, interop, intro, legacy, moduleContext, name, noConflict, onwarn, output, outro, paths, plugins, preferConst, preserveSymlinks, pureExternalModules, sourcemap, sourcemapFile, strict, targets, treeshake, watch"
				);
			}
		);
	});
