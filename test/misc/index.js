		...
		return rollup.rollup({ input: 'x', onwarn, plUgins: [], plugins: [loader({ x: `console.log( 42 );` })] }).then(
			() => {
				assert.deepEqual(
					...
					[{
						code: 'UNKNOWN_OPTION',
						message: 'Unknown option found: plUgins. Allowed keys: input, legacy, treeshake, acorn, context, moduleContext, plugins, onwarn, watch, cache, preferConst, experimentalDynamicImport, preserveSymlinks, includeMissingExports, includeAllNamespacedInternal, includeNamespaceConflicts, entry, external, extend, amd, banner, footer, intro, format, outro, sourcemap, sourcemapFile, name, globals, interop, legacy, freeze, indent, strict, noConflict, paths, exports, file, srcDir, destDir, preserveModules, excludedModules, pureExternalModules'
					}]
				);
			}
		);
